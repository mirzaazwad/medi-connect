...
import { useStripsAddBrand } from "../hooks/useStripsAddBrand";
import { CFormSwitch } from "@coreui/react";
import Type from "../../../assets/ts/medicine-type";
import Category from "../../../assets/ts/medicine-category";

const StripsForm = () => {
  const {medicine,stripInformation,utilities}=useStripsAddBrand();
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="d-flex">
            ...
            <input
                type="number"
                value={stripInformation.stripsPerBox}
                onChange={(e) =>
                  e.target.value!==""?stripInformation.setStripsPerBox(Number(DOMPurify.sanitize(e.target.value))):
                  stripInformation.setStripsPerBox(undefined)
                }
                required
              />
      ...
      <input
          type="number"
          value={stripInformation.pcsPerStrip}
          onChange={(e) =>
            e.target.value!==""?stripInformation.setPcsPerStrip(Number(DOMPurify.sanitize(e.target.value))):
            stripInformation.setPcsPerStrip(undefined)
          }
          required
        />
